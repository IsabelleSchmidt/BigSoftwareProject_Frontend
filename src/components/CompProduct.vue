  ...
  setup(props, context) {
    ...
    const productlist = computed(() => {
      ...
      if (filter.room === "all" && filter.producttype === "all") {
        ...
        return allproductslist.value.filter(
          (p) => p.roomType === filter.room?.toString()
        );
      } else if (filter.room === "all" && filter.producttype !== "all") {
        return allproductslist.value.filter(
          (p) => p.productType === filter.producttype?.toString()
        );
      } else {
        q.room.value = route.query.room;
        q.producttype.value = route.query.producttype;
        ...
        return allproductslist.value.filter(
          (p) =>
            p.productType === filter.producttype?.toString() &&
            p.roomType === filter.room?.toString()
        );
      }
    });
